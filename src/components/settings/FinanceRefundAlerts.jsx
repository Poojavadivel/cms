export default function FinanceRefundAlerts({
  title = 'Refund Request Alerts',
  description,
  values,
      ...
      <div className="role-settings-toggle-list">
        <label className="role-settings-toggle">
          <span className="role-settings-toggle-copy">
            <strong>Enable Refund Request Alerts</strong>
            <small>Notifies you whenever a new refund request is submitted.</small>
          </span>
          <span className="role-settings-toggle-control">
            <input
              type="checkbox"
              checked={Boolean(values.refundRequests)}
              onChange={(event) => onToggle('refundRequests', event.target.checked)}
            />
